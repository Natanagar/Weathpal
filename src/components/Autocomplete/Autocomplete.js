	...
	const handleChanges = (changes) => {
		if (changes.type === '__autocomplete_click_item__') {
			...
			if (!from) {
				...
				store.dispatch({ type: 'AUTOCOMPLETE_SELECTED_CURRENCY_FROM', to });
			}
		}
		...
		<div>
			...
			>
				{({
					...
					<div>
						<label htmlFor="input" {...getLabelProps()}>
							{direction}
						</label>
						<input {...getInputProps()} />
						{open ? (
							<Button as GrayButton {...getToggleButtonProps()} data={'open'} />
						) : (
							<Button onClick={clearSelection} data={'close'} />
						)}
